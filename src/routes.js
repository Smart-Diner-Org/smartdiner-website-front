...
import CreateRestaurant from './Admin_Panel/SmartDinerSuperAdmin/CreateRestaurant';

ReactGA.initialize(`${process.env.GA_INIT_API_KEY}`);


export default (  
  <Router>
    <Switch>
      <UnProtectedRoute  path="/admin-panel" component={Admin_Panel} />
      <UnProtectedRoute  path="/createOfflineOrders" component={OfflineOrder} />
      <UnProtectedRoute  path="/create-restaurant" component={CreateRestaurant} />
      <UnProtectedRoute path="/about-us" component={()=>componentWrapper(About)} />
      <UnProtectedRoute path="/contact" component={()=>componentWrapper(Contact)} />
      <UnProtectedRoute path="/login" component={()=>componentWrapper(Login)} />
      <UnProtectedRoute path="/signup" component={()=>componentWrapper(SignIn)} />
